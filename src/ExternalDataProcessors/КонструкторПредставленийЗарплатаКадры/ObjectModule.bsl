...
// в формате файлов *.descriptions.json репозитория SharedQuerySchemesHRM:
//  {
//    "Подсистема": "ИмяПодсистемы",
//    "Представление": "ИмяПредставления",
...
Процедура ЗаполнитьТекстовыеОписанияПредставлений(ОписанияПредставлений)
	...
	Для Каждого КлючИЗначение Из ОписанияПредставлений Цикл
		...
		ТекстОписания = ТекстовыеОписания["ОписаниеПредставления"];
		Если ТипЗнч(ТекстОписания) = Тип("Строка") Тогда
			ОписаниеПредставления.Описание = ТекстОписания;
		КонецЕсли;

		ТекстовыеОписанияПолей = ТекстовыеОписания["Поля"];
		Если ТипЗнч(ТекстовыеОписанияПолей) <> Тип("Массив") Тогда
			Продолжить;
		КонецЕсли;

		Для Каждого ТекстовоеОписаниеПоля Из ТекстовыеОписанияПолей Цикл
...
//
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение, Неопределено - Содержимое макета описаний, см. ЗаполнитьТекстовыеОписанияПредставлений.
...
Функция ТекстовыеОписанияПредставления(ИмяПредставления)
	...
	Если ЭтотОбъект.Метаданные().Макеты.Найти(ИмяМакета) = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;

	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(ПолучитьМакет(ИмяМакета).ПолучитьТекст());
	// Чтение в соответствие, а не в структуру: файл описаний может содержать служебный ключ "$schema".
	ТекстовыеОписания = ПрочитатьJSON(ЧтениеJSON, Истина);
	ЧтениеJSON.Закрыть();
...
//   * Подсистема, Имя, ЕстьИсточникДанных, ЕстьФильтр, Фильтр, ФильтрОбязателен, ДоступныОтборы,
//     ЕстьПрограммныйИнтерфейс - соответствуют полям описания представления
