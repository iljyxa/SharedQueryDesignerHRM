...
// Загружает описания представлений из хранилища общих настроек, если версия конфигурации
// и обработки совпадают с текущими.
//
// Возвращаемое значение:
//  Соответствие, Неопределено - Описания представлений или Неопределено, если кэш недоступен.
&НаСервере
...
//  ДополнительныеПараметры - Структура, Массив - Дополнительные параметры оповещения.
&НаКлиенте
...
Процедура УстановитьУсловноеОформление()
	...
	// Ранее условное оформление задавалось в конструкторе формы (ConditionalAppearance.dcssca).
	// Перенесено в код для удобства сопровождения и контроля версий.

	// Выделение обязательных параметров подсистемы.
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементОформления.Отбор,
		...
		Истина);
	// В исходном оформлении ЦветФона=AntiqueWhite отмечен как не используемый (dcscor:use=false),
	// поэтому выделение выполнено жирным шрифтом. При необходимости раскомментировать:
	//ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона", WebЦвета.AntiqueWhite);
