...
&НаКлиенте
Асинх Процедура ИмпортПредставленияИзТекстаЗапроса(Команда)
	
	ТекстПодсказки = "Вставьте текст запроса представления";
	
	ТекстЗапроса = Ждать ВвестиСтрокуАсинх("", ТекстПодсказки, , Истина);
...
&НаСервере
Процедура ИнициализироватьОписанияПредставлений(ПервичнаяИнициализацияФормы = Ложь)
...
Функция ТекстЗапросаПредставления(Знач ИмяПредставления,
	...
	Для Каждого ВыбранныйПараметр Из ВыбранныеПараметры Цикл
		...
		МассивОтборов.Добавить(ВыбранныйПараметрВВыражениеЯзыкаЗапроса(ВыбранныйПараметр, РазделительПараметров));
	КонецЦикла;
...
Функция ВыбранныйПараметрВВыражениеЯзыкаЗапроса(ВыбранныйПараметр, РазделительПараметров = "")
	...
	Если ВыбранныйПараметр.Отбор Тогда
		...
		Возврат СтрШаблон(ШаблонОтбора,
						  "ЛевоеЗначение" + ?(РазделительПараметров = "", "", "_" + РазделительПараметров),
						  """" + ВыбранныйПараметр.Имя + """")
				+ Символы.ПС
				+ Символы.Таб
				+ "И "
				+ СтрШаблон(ШаблонОтбора,
							"ВидСравнения" + ?(РазделительПараметров = "", "", "_" + РазделительПараметров),
							ЗначениеПараметраВВыражениеЯзыкаЗапроса(ВыбранныйПараметр.ВидСравнения))
				+ Символы.ПС
				+ Символы.Таб
				+ "И "
				+ СтрШаблон(ШаблонОтбора,
							"ПравоеЗначение" + ?(РазделительПараметров = "", "", "_" + РазделительПараметров),
							?(ВыбранныйПараметр.ЗначениеЭтоВыражение,
