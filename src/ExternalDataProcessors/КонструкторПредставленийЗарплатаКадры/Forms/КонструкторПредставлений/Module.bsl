...
&НаКлиенте
Асинх Процедура ПредставленияПослеАктивизацииСтроки()
...
&НаКлиенте
Асинх Процедура СформироватьТекстЗапроса(Команда)
	ТекстЗапроса = СформироватьТекстЗапросаНаСервере();
	Ждать ВвестиСтрокуАсинх(ТекстЗапроса, , , Истина);
КонецПроцедуры

&НаКлиенте
Процедура РедакторОписаний(Команда)
	ОткрытьФорму("ВнешняяОбработка.КонструкторПредставленийЗарплатаКадры.Форма.РедакторОписанийПредставлений",
				 ,
				 ЭтотОбъект);
КонецПроцедуры
...
Процедура ПредставленияИсточникДанныхПриИзмененииНаСервере()
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(ИмяВТФильтра, "Период");

	Если ВРег(Представление.Имя) = ВРег("ТаблицаРегистра") Тогда
		Запрос = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросВТИмяРегистра(Представление.ИсточникДанных,
																	Истина,
																	ОписаниеФильтра);
	ИначеЕсли ВРег(Представление.Имя) = ВРег("СрезПоследних") Тогда
		Запрос = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросВТИмяРегистраСрез(Представление.ИсточникДанных,
																		Истина,
																		ОписаниеФильтра,
																		,
																		Истина);
