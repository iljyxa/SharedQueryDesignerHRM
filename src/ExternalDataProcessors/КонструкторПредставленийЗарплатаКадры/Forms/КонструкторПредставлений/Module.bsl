...
Процедура ИнициализироватьОписанияПредставлений(Знач ЗаполнитьИзКэша = Истина)
	...
	Для Каждого КлючИзначение Из ОписанияПредставлений Цикл
		// При заполнении из кэша структура параметров могла измениться после описания представления.
		// В этом случае необходимо заполнить параметры заново.
		// Если описания не кэшировались ранее, то параметры будут заполнены корректно.
		Если ЗаполнитьИзКэша Тогда
			Описание = Обработка.НовоеОписаниеПредставления();
			ЗаполнитьЗначенияСвойств(Описание, КлючИзначение.Значение);
		Иначе
